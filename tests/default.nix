...

{
  ...
  # Test --safe-chain-version flag on pip wrapper
  pip-version-flag = pkgs.runCommand "test-pip-version-flag" {
    nativeBuildInputs = [ (safeChain.wrapPython pkgs.python3) ];
  } ''
  ...
  node-wrapper-symlinks = pkgs.runCommand "test-node-wrapper-symlinks" {
    ...
    echo "PASS: all Node.js wrapper symlinks exist"
    touch $out
  '';

  # Test wrapper symlinks exist for Python package managers
  python-wrapper-symlinks = pkgs.runCommand "test-python-wrapper-symlinks" {
    nativeBuildInputs = [ (safeChain.wrapPython pkgs.python3) ];
  } ''
